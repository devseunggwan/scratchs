...
def get_weekdays_in_month(month_range: int = 6, date_format: str = "%Y%m%d") -> list[str]:
    ...
    __last_month_day = (__now + relativedelta(months=month_range + 1)).replace(
...
def get_business_days_in_holidays(
    holidays: list[str] = [], month_range: int = 6, date_format: str = "%Y%m%d"
) -> dict[str, bool]:
