...
if __name__ == "__main__":
    ...
    print("(prod - dev) 컬럼 목록: ", set(df_1.columns) - set(df_2.columns))
    print("(dev - prod) 컬럼 목록: ", set(df_2.columns) - set(df_1.columns))
    ...
    # Use different naming columns to compare
    appendix_column_pairs = []
    column_pairs.extend(appendix_column_pairs)

    print(f"\n[비교 대상 컬럼] 총 {len(column_pairs)}개의 컬럼 쌍을 비교합니다.")
    ...
    if not column_pairs:
        ...
        for base_name, prod_col, dev_col in column_pairs:
            ...
            # NaN을 포함한 값 비교
            is_equal = (df[prod_col] == df[dev_col]) | (
                df[prod_col].isna() & df[dev_col].isna()
            )
        ...
        if all_match:
            ...
            for detail in diff_details:
                ...
                print(
                    f"  - 데이터 타입: prod({detail['prod_dtype']}) vs dev({detail['dev_dtype']})"
                )
                print("  - 샘플 데이터 (최대 3개):")
